...
const submitBtn = document.querySelector("#auth")
submitBtn.addEventListener('click', function() {
    
    if (submitBtn.innerText === 'Log In') {
        submitBtn.innerText = 'Log Out';
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    var alertMessageInput = document.getElementById('alert-message');
    var sendAlertButton = alertMessageInput.nextElementSibling; 

    sendAlertButton.addEventListener('click', function(event) {
        event.preventDefault(); 
        var message = alertMessageInput.value;
        alert(message || 'No message entered');
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    var addButton = document.getElementById('item-adder');
...
document.addEventListener('DOMContentLoaded', function() {
   let blueButton = document.getElementById('blue');
    ...
    blueButton.addEventListener('click', function() {
      let elements = document.querySelectorAll('.changes-colors');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    inputField = document.getElementById('factorial-input');
    calculateButton = inputField.nextElementSibling; 
...
document.addEventListener('DOMContentLoaded', function() {
    wordForm = document.getElementById('recommend-word');

     feedbackElement = document.querySelector('p.form-feedback');
if (feedbackElement) {
    feedbackElement.remove();
}
    
    feedbackDiv = document.createElement('div');
    wordForm.appendChild(feedbackDiv); 

    wordForm.addEventListener('submit', function(event) {
        event.preventDefault(); 
         word = wordForm.querySelector('#word').value;

        if (word.length >= 3) {
            feedbackDiv.textContent = 'Thanks for your submission!';
            feedbackDiv.style.color = 'green';
        } else {
            feedbackDiv.textContent = 'The word must be at least 4 characters long.';
